...
use super::{cweamcat_lair_layout::CweamcatLairInfo, factory_1_layout::Factory1Info, DoorCollider, EntityInfo, FloorAssetType, FloorInfo, FloorModification, LevelInfo, TransitionCollider};
...
impl LevelInfo for FactoryTransitionInfo {
    fn get_floor_info(&self, cweampuff: &crate::Cweampuff) -> Box<[FloorInfo]> {
        let mut floors = vec![
            ...
            FloorInfo { position: Vec3::new(350.0, -1200.0, 1.0), size: Vec2::new(2400.0, 300.0), breakable_wall: None, floor_asset: FloorAssetType::Forest },
    ...
    fn get_transitions_info(&self, cweampuff: &crate::Cweampuff) -> Option<Box<[TransitionCollider]>> {
        let transitions = vec![
            ...
            TransitionCollider { exit_index: 0, safe_position: Vec3::new(1150.0, 100.0, CWEAMPUFF_Z_INDEX), transition_to_level: Level::Factory1(Factory1Info), floor_info: EntityInfo { position: Vec3::new(1350.0, 150.0, 2.0), size: Vec2::new(100.0, 200.0) }  },
        ];
